...
LedChannel = m5stack_chain_key_ns.enum("LedChannel", is_class=True)

CONF_CHAIN_KEY_ID = "chain_key_id"
...
CONFIG_SCHEMA = output.FLOAT_OUTPUT_SCHEMA.extend(
    {
        cv.Required(CONF_ID): cv.declare_id(ChainKeyLedOutput),
        cv.Required(CONF_CHANNEL): cv.enum(CHANNEL_MAP),
        cv.GenerateID(CONF_CHAIN_KEY_ID): cv.use_id(ChainKeyBinarySensor),
    }
)
...
async def to_code(config):
    ...
    parent = await cg.get_variable(config[CONF_CHAIN_KEY_ID])
