...
static const char *const TAG = "m5stack_chain_tof";

void ChainToFSensor::setup() {
  ESP_LOGCONFIG(TAG, "Setting up M5Stack Chain ToF sensor (device_id=%u)...", this->device_id_);
}

void ChainToFSensor::update() {
  uint16_t distance = 0;
  ChainStatus status = this->get_distance_(this->device_id_, &distance, 100);

  if (status == CHAIN_OK) {
    this->publish_state(distance);
  } else {
    ESP_LOGW(TAG, "Failed to read ToF distance (status=0x%02X)", static_cast<uint8_t>(status));
  }
}

ChainStatus ChainToFSensor::get_distance_(uint8_t id, uint16_t *distance, uint32_t timeout_ms) {
  if (distance == nullptr) {
  ...
  if (this->acquire_mutex_()) {
    ...
    this->send_packet_(id, CHAIN_TOF_GET_DISTANCE, this->cmd_buffer_, this->cmd_buffer_size_);

    if (this->wait_for_data_(id, CHAIN_TOF_GET_DISTANCE, timeout_ms)) {
      if (this->check_packet_(this->return_packet_, this->return_packet_size_)) {
        *distance = (static_cast<uint16_t>(this->return_packet_[7]) << 8) | this->return_packet_[6];
      } else {
...
void ChainToFSensor::release_mutex_() { this->mutex_locked_ = false; }

void ChainToFSensor::send_packet_(uint16_t id, uint8_t cmd, const uint8_t *buffer, uint16_t size) {
...
bool ChainToFSensor::wait_for_data_(uint16_t id, uint8_t cmd, uint32_t timeout_ms) {
  uint32_t start = millis();
...
void ChainToFSensor::read_buffer_() {
...
bool ChainToFSensor::process_buffer_data_(uint16_t id, uint8_t cmd) {
...
bool ChainToFSensor::check_packet_(const uint8_t *buffer, uint16_t size) const {
...
uint8_t ChainToFSensor::calculate_crc_(const uint8_t *buffer, uint16_t size) const {
  uint8_t crc8 = 0;
