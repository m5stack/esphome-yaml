  ...
  void write_state(float state) override {
    ...
    switch (channel_) {
      ...
      case RGB_BLUE: {
        ...
        // 重要：每次都读取设备当前颜色（和 unit_step16 一样），避免多个 output 实例之间的竞态条件
        uint8_t r = 0, g = 0, b = 0;
        if (parent_->get_rgb_color(&r, &g, &b, &op_status) != CHAIN_OK) {
          ESP_LOGW(TAG_ANGLE_OUTPUT, "Failed to get current RGB, using defaults");
          r = g = b = 0;
        }
        ...
        ESP_LOGD(TAG_ANGLE_OUTPUT, "Setting RGB color: R=%u, G=%u, B=%u", r, g, b);
